...
{
    let offer = PeerMessage::offer_file_v1(file_name, file_size);
    send_blob(
        wormhole,
        relay_hints,
        file,
...
/// Send an already-zipped directory as a protocol-v1 `directory` offer
/// (`mode: "zipfile/deflated"`), the same wire format the Python reference
/// client uses for `wormhole send <dir>`. The receiver learns the folder name,
/// file count and unpacked size from the offer, and receives `zip_size` bytes
/// of zip data.
pub(crate) async fn send_zipped_directory<F, G, H>(
    wormhole: Wormhole,
    relay_hints: Vec<transit::RelayHint>,
    zip: &mut F,
...
where
    ...
    H: FnMut(u64, u64) + 'static,
{
    // "zipfile/deflated" is the only mode the reference implementation emits
    // or accepts (its receiver hard-rejects everything else).
    let offer = PeerMessage::offer_directory_v1(
        dir_name,
        "zipfile/deflated",
        zip_size,
        num_bytes,
...
/// The shared body of [`send_file`] and [`send_zipped_directory`]: send one
/// offer message and then `file_size` bytes over transit.
async fn send_blob<F, G, H>(
    ...
    H: FnMut(u64, u64) + 'static,
{
    let run = Box::pin(async {
        ...
        // Send the offer message (file or zipped directory).
        tracing::debug!("Sending offer");
        wormhole.send_json(&offer).await?;
        ...
        let (filename, filesize, directory) =
            match wormhole.receive_json::<PeerMessage>().await??.check_err()? {
                PeerMessage::Offer(offer_type) => match offer_type {
                    v1::OfferMessage::File { filename, filesize } => (filename, filesize, None),
                    v1::OfferMessage::Directory {
                        ...
                        ensure!(mode == "zipfile/deflated", TransferError::UnsupportedOffer);
                        /* Keep the historic `<dirname>.zip` name so callers that
                         * ignore the directory metadata still save a usable file */
                        let filename = format!("{dirname}.zip");
                        let directory = DirectoryOfferInfo {
                            dir_name: dirname,
                            num_files: numfiles,
                            num_bytes: numbytes,
                        };
                        (filename, zipsize, Some(directory))
                    },
...
pub struct ReceiveRequest {
    ...
    /// Present when the offer was a zipped directory rather than a file
    directory: Option<DirectoryOfferInfo>,
...
impl ReceiveRequest {
    ...
    /// Directory-offer metadata, when the peer offered a zipped folder
    /// instead of a single file.
    ///
    /// When this is `Some`, the payload written by
    /// [`accept`](ReceiveRequest::accept) is a zip archive of the offered
    /// folder ([`file_name`](ReceiveRequest::file_name) is synthesized as
    /// `<dirname>.zip` for backwards compatibility).
    pub fn directory_offer(&self) -> Option<&DirectoryOfferInfo> {
